...
namespace AniWorld.Board
{
    public class BoardManager : MonoBehaviour
    {
        ...
        [SerializeField] private Tilemap terrainOverlayTilemap;
        ...
        [SerializeField] private BoardVisualProfile visualProfile;
        [SerializeField] private bool useRandomTileVariants = true;

        [Header("Tile Visual Layers")]
        [SerializeField] private bool enableTileThickness = true;
        [SerializeField] private bool enableTileShadows = true;
        [SerializeField] private bool enableTerrainOverlays = false;
        [SerializeField] private bool enableTerrainDecorations = true;
        ...
        public bool EnableTileThickness
        {
            get => enableTileThickness;
            set
            {
                if (enableTileThickness == value)
                {
                    return;
                }

                enableTileThickness = value;
                RefreshAllTerrainTiles();
            }
        }

        public bool EnableTileShadows
        {
            get => enableTileShadows;
            set
            {
                if (enableTileShadows == value)
                {
                    return;
                }

                enableTileShadows = value;
                RefreshAllTerrainTiles();
            }
        }

        public bool EnableTerrainOverlays
        {
            get => enableTerrainOverlays;
            set
            {
                if (enableTerrainOverlays == value)
                {
                    return;
                }

                enableTerrainOverlays = value;
                RefreshAllTerrainTiles();
            }
        }

        public bool EnableTerrainDecorations
        {
            get => enableTerrainDecorations;
            set
            {
                if (enableTerrainDecorations == value)
                {
                    return;
                }

                enableTerrainDecorations = value;
                RefreshAllTerrainTiles();
            }
        }

        public BoardVisualProfile VisualProfile => visualProfile;
        public Tilemap ShadowTilemap => shadowTilemap;
        public Tilemap SideTilemap => sideTilemap;
        public Tilemap TerrainTilemap => terrainTilemap;
        public Tilemap TerrainOverlayTilemap => terrainOverlayTilemap;
        ...
        public void ClearBoard()
        {
            ...
            ClearTilemap(shadowTilemap);
            ClearTilemap(sideTilemap);
            ClearTilemap(terrainTilemap);
            ClearTilemap(terrainOverlayTilemap);
        ...
        public void RemoveCell(Vector2Int pos)
        {
            ...
            SetTileSafe(shadowTilemap, tilePos, null);
            SetTileSafe(sideTilemap, tilePos, null);
            SetTileSafe(terrainTilemap, tilePos, null);
            SetTileSafe(terrainOverlayTilemap, tilePos, null);
        ...
        {
            Vector3 worldCenter = GetCellWorldCenter(pos);
            return visualProfile != null ? worldCenter + visualProfile.UnitPlacementOffset : worldCenter;
        }
        ...
        private void RefreshTerrainTile(Vector2Int pos, TerrainType terrain)
        {
            ...
            TileBase sideTile = null;
            TileBase shadowTile = null;
            TileBase decorationTile = null;
            if (tileDatabase != null)
            {
                terrainTile = useRandomTileVariants
                    ? tileDatabase.GetRandomTerrainTile(terrain)
                    : tileDatabase.GetTerrainTile(terrain);
                if (enableTerrainOverlays)
                {
                    overlayTile = useRandomTileVariants
                        ? tileDatabase.GetRandomOverlayTile(terrain)
                        : tileDatabase.GetOverlayTile(terrain);
                }

                if (enableTileThickness)
                {
                    sideTile = useRandomTileVariants
                        ? tileDatabase.GetRandomSideTile(terrain)
                        : tileDatabase.GetSideTile(terrain);
                }

                if (enableTileShadows)
                {
                    shadowTile = useRandomTileVariants
                        ? tileDatabase.GetRandomShadowTile(terrain)
                        : tileDatabase.GetShadowTile(terrain);
                }

                if (enableTerrainDecorations)
                {
                    decorationTile = useRandomTileVariants
                        ? tileDatabase.GetRandomDecorationTile(terrain)
                        : tileDatabase.GetDecorationTile(terrain);
                }
            }

            SetTileSafe(shadowTilemap, tilePos, shadowTile);
            SetTileSafe(sideTilemap, tilePos, sideTile);
            SetTileSafe(terrainTilemap, tilePos, terrainTile);
            SetTileSafe(terrainOverlayTilemap, tilePos, overlayTile);
            SetTileSafe(decorationTilemap, tilePos, decorationTile);
        }

        public void ApplyVisualProfile()
        {
            ...
            ApplyLocalPosition(shadowTilemap, visualProfile.ShadowTilemapOffset);
            ApplyLocalPosition(sideTilemap, visualProfile.SideTilemapOffset);
            ApplyLocalPosition(terrainTilemap, visualProfile.TerrainTilemapOffset);
            ApplyLocalPosition(terrainOverlayTilemap, visualProfile.TerrainOverlayTilemapOffset);
            ApplyLocalPosition(decorationTilemap, visualProfile.DecorationTilemapOffset);
            ApplyLocalPosition(highlightTilemap, visualProfile.HighlightTilemapOffset);
            ApplyLocalPosition(overlayTilemap, visualProfile.OverlayTilemapOffset);
        }

        public void RefreshAllTerrainTiles()
        {
            if (cells.Count == 0)
            {
                ClearTilemap(shadowTilemap);
            ...
            foreach (BoardCell cell in cells.Values)
            {
                if (cell != null)
                {
                    RefreshTerrainTile(cell.Position, cell.Terrain);
                }
            }
        }

        private static void ApplyLocalPosition(Tilemap tilemap, Vector3 localPosition)
        {
            if (tilemap != null)
            {
