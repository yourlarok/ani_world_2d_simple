...
namespace AniWorld.Board.Generation
{
    public class BoardRandomGenerator : MonoBehaviour
    {
        ...
        [ContextMenu("Save Last Result As Asset")]
        public void SaveLastResultAsAsset()
        {
            if (LastResult == null)
            {
                Debug.LogWarning("Generate a map before saving it as an asset.", this);
                return;
            }

#if UNITY_EDITOR
            defaultMap = BoardGeneratedMapAssetUtility.SaveGeneratedMap(LastResult, saveFolder, saveAssetName);
#else
            Debug.LogWarning($"Saving generated maps as assets is only available in the Unity Editor. Requested path: {saveFolder}/{saveAssetName}.asset", this);
#endif
        }
